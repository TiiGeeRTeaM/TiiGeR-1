...
-----------------------------------------
local function delmsg (TiiGeR,Arashwm)
    msgs = TiiGeR.msgs 
    for k,v in pairs(Arashwm.messages_) do
        msgs = msgs - 1
        tdcli.deleteMessages(v.chat_id_,{[0] = v.id_}, dl_cb, cmd)
        if msgs == 1 then
            tdcli.deleteMessages(Arashwm.messages_[0].chat_id_,{[0] = Arashwm.messages_[0].id_}, dl_cb, cmd)
            return false
        end
    end
    tdcli.getChatHistory(Arashwm.messages_[0].chat_id_, Arashwm.messages_[0].id_,0 , 100, delmsg, {msgs=msgs})
end
-----------------------------------------
local function Arwm(msg)
